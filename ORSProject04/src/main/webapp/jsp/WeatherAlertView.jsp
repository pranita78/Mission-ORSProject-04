...
<%@page import="com.sunilos.p4.ctl.WeatherAlertCtl"%>
			...
			<form action="<%=ORSView.WEATHER_ALERT_CTL%>" method="POST">
				...
				<div class="mb-3">
					<label class="form-label fw-semibold">Alert Code <span
						class="text-danger">*</span></label> <input type="text" name="alertCode"
						class="form-control" maxlength="100"
						value="<%=DataUtility.getStringData(bean.getAlertCode())%>">
					<div class="text-danger small mt-1"><%=ServletUtility.getErrorMessage("alertCode", request)%></div>
				</div>

				<div class="mb-3">
					<label class="form-label fw-semibold">City Name <span
						class="text-danger">*</span></label> <input type="text"
						name="cityName" class="form-control" maxlength="200"
						value="<%=DataUtility.getStringData(bean.getCityName())%>">
					<div class="text-danger small mt-1"><%=ServletUtility.getErrorMessage("cityName", request)%></div>
				</div>

				<div class="mb-3">
					<label class="form-label fw-semibold">Temperature <span
						class="text-danger">*</span></label> <input type="text" name="temperature"
						class="form-control" maxlength="20"
						value="<%=DataUtility.getStringData(bean.getTemperature())%>">
					<div class="text-danger small mt-1"><%=ServletUtility.getErrorMessage("temperature", request)%></div>
				</div>
